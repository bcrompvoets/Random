...
# YSO_EG_Stars Valid
X_va = np.load("../Data/c2d_Inputs_CORES_Valid.npy") # Load input data
Y_va = np.load("../Data/c2d_Targets_CORES_Valid.npy") # Load target data
X_va = np.float32(X_va)
Y_va = np.float32(Y_va)
inp_va, tar_va = replicate_data_single(X_va, Y_va, [314,212,4269])

# YSO_EG_Stars Test
X_te = np.load("../Data/Rap_Inputs_Test.npy") # Load input data
Y_te = np.load("../Data/Rap_Targets_Test.npy") # Load target data
X_te = np.float32(X_te)
Y_te = np.float32(Y_te)
inp_te, tar_te = replicate_data_single(X_te, Y_te, [376,155,262]) # We still use this as it shuffles the data
...
if __name__ == '__main__':
    ...
    epochs = 3000
    filepath = "../MLP_Runs_Results/YSO_EG_Stars/c2d_Train/"
